...
@cache.memoize()
def load_data(date, chunk_size=500000):
    filename = f"{date}.csv"
    try:
        df_list = []
...
app.layout = html.Div([
    html.H1("Performance Dashboard",
            style={'textAlign': 'center', 'color': '#2c3e50', 'font-family': 'Helvetica, Arial, sans-serif',
                   'margin-bottom': '30px'}),
    ...
    ], style={'display': 'flex', 'justifyContent': 'center', 'alignItems': 'center', 'backgroundColor': '#ecf0f1',
...
def update_dashboard(selected_date, n_clicks):
    ...
    if df.empty:
        return html.Div("No data available for the selected date.",
                        style={'textAlign': 'center', 'marginTop': '20px', 'color': '#e74c3c', 'fontSize': '18px'})
    ...
    if n_clicks % 2 == 0:
        return [
            ...
            ], style={'margin': '20px', 'padding': '20px', 'backgroundColor': '#ffffff', 'borderRadius': '10px',
                ...
                html.Div([
                    dash_table.DataTable(
                        ...
                        columns=[{"name": i, "id": i} for i in
                                 ['OptionEMMId', 'UnderlyingEMMId', 'T2-T1', 'T3-T2', 'T4-T3', 'T5-T4', 'T5-T2',
                                  'Insert/Update']],
                        data=df.to_dict('records'),
                        page_size=5,
                        style_cell={
        ...
        return html.Div([
            ...
            html.Div([
                html.H3("T2 Timestamp Analysis (Second Precision)",
                        style={'color': '#34495e', 'textAlign': 'center', 'fontSize': '22px'}),
